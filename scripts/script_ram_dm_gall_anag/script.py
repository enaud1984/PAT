...
class ScriptProcess:
    def process(self, req):
        ...
        df_upd = df_1.join(df_2, cond, 'inner').select(df_1["*"], lit("U").alias("OPERATION"))
        df_ins = df_1.join(df_2, cond, "left_anti").select(df_1["*"], lit("I").alias("OPERATION"))
        df_del = df_2.join(df_1, cond, "left_anti").select(df_2["*"], lit("D").alias("OPERATION"))
