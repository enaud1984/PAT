...
schema = [
    {
        "IDNodoDiRiferimento#id_nodo_riferimento": "",
        "Descrizione#descrizione": "",
        "Regione#regione": "",
        "DataInizioValidita#data_inizio_validita(timestamp|%Y-%m-%dT%H:%M:%SZ)": None,
        "DataFineValidita#data_fine_validita(timestamp|%Y-%m-%dT%H:%M:%SZ)": None,
...
class ScriptProcess:
    def process(self, req):
        ...
        logging.info(f"PicNodoDiRiferimento - len(list_) - {len(list_)}")
        logging.info(f"PicNodoDiRiferimento - list_ - {list_}")
